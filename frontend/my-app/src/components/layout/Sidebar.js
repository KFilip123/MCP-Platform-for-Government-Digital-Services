...
function ChatItem({ session, onRename, onDelete }) {
    ...
    <aside className="w-60 h-screen bg-gray-900 border-r border-gray-800 flex flex-col">
      ...
      {sessions.length > 0 && (
        <div className="px-3 py-2 flex-1 min-h-0 flex flex-col">
          <p className="text-gray-600 text-xs font-semibold uppercase tracking-wider px-2 mb-2">Recent Chats</p>
          ...
          <div className="flex-1 min-h-0 overflow-y-auto">
            ...
            {filtered.length === 0 && (
              <p className="text-gray-600 text-xs px-2 py-1">No matches</p>
            )}
          </div>
        </div>
      )}
