"""
institutions/mon/tools/scholarships.py
────────────────────────────────────────────────────────────────────────────────
Tools for browsing MON scholarships (Стипендии) from mon.gov.mk.

Available tools:
  • list_mon_scholarships([page_url])        — list scholarships, paginatable
  • get_mon_scholarship_details(url)         — full details for one scholarship
"""

from institutions.mon.tools._scraper import scrape_listing, scrape_detail

_SCHOLARSHIPS_URL = "https://mon.gov.mk/mk-MK/konkursi-i-stipendii/stipendii-mon"
_SECTION = "stipendii-mon"


def list_mon_scholarships(page_url: str | None = None) -> dict:
    """
    List MON scholarships (Стипендии) scraped from mon.gov.mk.

    Args:
        page_url: Unused — kept for backwards compatibility. All pages are
                  fetched automatically.

    Returns:
        {
            "scholarships": list of {
                "title":       str,
                "date":        str,   # DD/MM/YYYY
                "url":         str,   # detail page URL
                "description": str,   # short excerpt
            },
            "total": int,
        }
    """
    items = scrape_listing(page_url or _SCHOLARSHIPS_URL, _SECTION)
    return {"scholarships": items, "total": len(items)}


def get_mon_scholarship_details(url: str) -> dict:
    """
    Fetch full details for a specific MON scholarship.

    Args:
        url: Full URL of the scholarship detail page, as returned in the
             "url" field of list_mon_scholarships().

    Returns:
        {
            "url":         str,
            "title":       str,
            "date":        str,         # DD/MM/YYYY publication date
            "body":        str,         # full scholarship text (up to 4000 chars)
            "attachments": list of {    # downloadable files (PDFs, docs, etc.)
                "name": str,
                "url":  str,
            },
        }
    """
    return scrape_detail(url)
